...
- **ImplementSweepWorkflow (agents#412)** — единственный путь, которым `accepted`
  ...
  кандидат, ПЕРЕЖИВШИЙ скан, логируется строкой
  `STRANDED service=... slug=... reason=...` независимо от того, был он
  сабмичен, уже свипается, отброшен cap'ом, исчерпанным pre-start-бюджетом или
  нечитаемым бюджетом. Отсев происходит РАНЬШЕ и строк `STRANDED` не даёт
  вовсе — такие записи в `scan.stranded` не попадают: это `pr:`, `unrunnable`,
  `blocked`, грейс-период, лиза и авторизация, все шесть ведут себя одинаково
  и уходят в `StrandedScanResult.skipped`, а карантин дополнительно в
  `.unauthorized`. Бюджет считается
...
- **Бюджет pre-start попыток.** `MAX_SWEEP_PRESTART_ATTEMPTS` ограничивает
  ...
  Второе ограничение, из того же ряда и срабатывающее НАМНОГО раньше:
  считаются только `_MAX_EXAMINED_PER_ID` (12) последних ранов каждого id, то
  есть значение — «pre-start-потери среди последних 12 ранов», а не за всё
  время. Это допуск ровно на `12 - MAX_SWEEP_PRESTART_ATTEMPTS` = 9
  незасчитанных ранов вперемешку, не гарантия: дальше окно вытесняет потерю и
  вердикт может откатиться с «исчерпан» на «в бюджете». Обход листинга
  ограничен сверху отдельно (потолок на чанк, пропорциональный его размеру),
  поэтому при очень длинном листинге counts — нижняя оценка, а id, до которого
  обход не дошёл, возвращается нулём, а НЕ опускается: опускать здесь значило
  бы вечно голодать его при стабильном порядке листинга. Retention-окно
  сбрасывается неделями, 12 ранов набираются за час пересабмитов — так что на
  практике бьёт именно второе.
