...
flowchart TB
    ...
    subgraph TMP["Temporal · ns=mctl-agents · queue=mctl-dev-loop"]
        ...
        INC["IncidentLoopWorkflow<br/>schedule 1h · ПРИОСТАНОВЛЕНО"]
        DOC["DocsDeltaWorkflow<br/>(по запросу)"]
    end

    subgraph ACT["Activities (в воркере, не в Argo)"]
        ...
        DDA["process_docs_delta_activity"]
    end

    subgraph EXT["Внешние системы"]
        API["mctl-api"]
        ARGO["Argo CWFT<br/>mctl-agents-investigate / -implement / -incidents"]
...
sequenceDiagram
    ...
    alt investigate не Succeeded
        ...
        T->>S: submit_and_wait("mctl-agents-implement", {service: target_repo, ...})
        S->>A: POST /operations/.../execute → poll
        S-->>T: WorkflowResult
        T->>E: record_execution("implementer", ...)
        T-->>OP: DevLoopResult(investigate, implement)
    end
```
...
stateDiagram-v2
    ...
    AwaitApproval --> ResolveImplementer: approve()
    ResolveImplementer --> Implement: service = repo этого issue
    Implement --> Done
    Failed --> [*]
    Done --> [*]
    ...
    note right of AwaitApproval
      ...
      approve() НЕ флипает .status.yaml:
      proposed → accepted руками в gitops,
      иначе implement-CWFT no-op (skipped_reason)
    end note
```
...
flowchart LR
    ...
    REC --> O["detect_orphans<br/>accepted/in-progress/implemented/review-fixing<br/>с открытым PR и без активного DevLoop → лог ORPHAN"]
    ...
    S3["schedule 1h<br/>ПРИОСТАНОВЛЕНО"] --> INC["IncidentLoopWorkflow"]
    ...
    DOC --> DD["process_docs_delta_activity<br/>deprecated/formatting_only → no-op<br/>иначе author_question → mctl-academy"]
```
...
- Temporal владеет **investigate → approve → implement**. Tier 3 (shepherd, ревью/мердж) в Temporal **не перенесён** — по-прежнему `run_shepherd.py` по крону; Reconcile лишь читает его состояние.
