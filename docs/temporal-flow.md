...
- **ImplementSweepWorkflow (agents#412)** — единственный путь, которым `accepted`
  ...
  сабмичен, отброшен cap'ом, исчерпанным pre-start-бюджетом или нечитаемым
  бюджетом. Отсев грейс-периодом, лизой и авторизацией происходит РАНЬШЕ: такие
  записи в `scan.stranded` не попадают вовсе и строки `STRANDED` не дают —
  они уходят в `StrandedScanResult.skipped`, а карантин дополнительно в
