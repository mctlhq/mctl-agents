...
- Argo-объект живёт ограниченное время, и TTL **не единый**: `investigate`,
  `implement`, `issue-poll` и `approve` — `secondsAfterCompletion: 86400`,
  `reconcile` и `shepherd` — `259200`, `run` — `1209600`. Ни
  `secondsAfterSuccess`, ни `secondsAfterFailure` не выставлены ни в одном
  шаблоне: успешные и упавшие раны живут одинаково. Результат нужно сохранять
  сразу (`record_execution`), перечитать Argo позже нельзя.
  Эти числа теперь сверяются с mctl-gitops автоматически —
  `_CWFT_BOUNDARIES` в `orchestrator/validate_manifest.py`; правка шаблона без
  правки Python красит CI (agents#149).
- Ретраи на incident-пути разведены по слоям и записаны в ADR 009: Argo
  ретраит саму работу, Temporal — только возобновление ожидания (бюджет
  попыток покупает пере-подключение, а не повторный запуск), расписание —
  следующую попытку. Поэтому `Failed` возвращается, а не бросается.
