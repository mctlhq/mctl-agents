...
# ---------------------------------------------------------------------------
def test_exit_rate_limited_is_52_and_collides_with_no_other_sentinel() -> None:
    """50 and 51 were taken by EXIT_CI_EVIDENCE_INSUFFICIENT (#423) and
    EXIT_VERIFICATION_BUDGET_EXHAUSTED (#430) while this change was in flight;
    a collision would make the shepherd classify a quota-exhausted run as a
    harness failure (bounded by MAX_HARNESS_FAILURES) instead of transient."""
    codes = {
        name: value
        for name, value in vars(run_implementer).items()
        if name.startswith("EXIT_") and isinstance(value, int)
    }
    assert run_implementer.EXIT_RATE_LIMITED == 52
    others = {n: v for n, v in codes.items() if n != "EXIT_RATE_LIMITED"}
    assert run_implementer.EXIT_RATE_LIMITED not in others.values(), others
...
def test_implement_one_skips_before_clone_when_window_is_open(tmp_path, monkeypatch) -> None:
    ...
    # A skip inside an open window is still rate-limited: the batch stops
